...
const normalizeFieldName = (field?: string) =>
  (field ?? "").toLowerCase().replaceAll(" ", "").replaceAll("-", "");
...
export const cardListToCsv = (cardList: Card[]) => {
  ...
  for (let cnt = 0; cnt < cardList.length; cnt++) {
    ...
    write(
      "Star Type",
      typeof star === "number" && star >= 0 && star <= 13 ? "number" : "text"
    );
    ...
    write(
      "External Info (JSON)",
      stringifedExternalInfo === "{}" ? "" : stringifedExternalInfo
    );

    if (
      artSource === "offline" ||
      (hasBackground && backgroundSource === "offline")
    ) {
      error = "offline-data";
    }
  ...
  return {
    value: [CsvStandardFieldList.join(","), valueList.join("\n")].join("\n"),
    error,
  };
};
...
const getCsvFieldReader = (
  row: (string | undefined)[],
  headerIndexMap: Record<CsvField, number | undefined>
) => {
...
const normalizeInt = (value: any, fallback: number) => {
  ...
  if (typeof value === "string")
    return isFinite(parseInt(value)) ? parseInt(value) : fallback;
  return fallback;
};
const normalizeFloat = (value: any, fallback: number) => {
  if (typeof value === "number") return isFinite(value) ? value : fallback;
  if (typeof value === "string")
    return isFinite(parseFloat(value)) ? parseFloat(value) : fallback;
  return fallback;
};
    ...
    return (
      data
        .map((row, index) => {
          ...
          const frame = (
            reader("Frame") ??
            reader("Background_Type") ??
            emptyCard.frame
          ).toLowerCase();
          const rightFrame = (
            reader("Right Frame") ?? emptyCard.rightFrame
          ).toLowerCase();
          const leftFrame = (
            reader("Left Frame") ?? emptyCard.leftFrame
          ).toLowerCase();
          const pendulumRightFrame = (
            reader("Bottom Right Frame") ?? emptyCard.pendulumRightFrame
          ).toLowerCase();

          const rawStar = reader("Star") ?? reader("Level/Rank") ?? "";
          const star =
            reader("Star Type") === "text"
              ? rawStar
              : parseInt(rawStar) <= 13 &&
                parseInt(rawStar) >= 0 &&
                reader("Star_Type") !== "2"
          ...
          const artFinish =
            reader("Art Finish") ??
            (reader("Art_Finish") ? `type${reader("Art_Finish")}` : "");
          ...
          const furiganaHelper = normalizeBoolean(
            reader("Use Furigana Helper"),
            emptyCard.furiganaHelper
          );
          const name = reader("Name") ?? "";
          const password = reader("Password") ?? "";
          const setId = reader("Set Id") ?? reader("Set_ID") ?? "";
          const starAlignment = (
            reader("Star Alignment") ?? emptyCard.starAlignment
          ).toLowerCase();
          const sticker = (
            reader("Sticker") ?? emptyCard.sticker
          ).toLowerCase();
          const subFamily = (
            reader("Spell/Trap Icon") ??
            reader("spell_type/trap_type") ??
            emptyCard.subFamily
          ).toUpperCase();
          ...
          const isLegacyCard = normalizeBoolean(
            reader("Is Legacy Card"),
            emptyCard.isLegacyCard
          );

          const isPendulum = normalizeBoolean(
            reader("Is Pendulum"),
            emptyCard.isPendulum
          );
          const pendulumFrame = (
            reader("Bottom Frame") ?? emptyCard.pendulumFrame
          ).toLowerCase();
          ...
          const pendulumSize = (reader("Pendulum Size") ??
            "medium") as PendulumSize;
          ...
          const condenseTolerant = (
            reader("Condense Rate") ?? emptyCard.effectStyle.condenseTolerant
          ).toLowerCase() as CondenseType;
          const effectUpSize = normalizeInt(
            reader("Effect Style - Upsize"),
            emptyCard.effectStyle.upSize
          );
          const pendulumEffectUpSize = normalizeInt(
            reader("Pendulum Effect Style - Upsize"),
            emptyCard.pendulumStyle.upSize
          );
          const effectFontStyle = (
            reader("Effect Style - Font Style") ??
            emptyCard.effectStyle.fontStyle
          ).toLowerCase();
          const pendulumEffectFontStyle = (
            reader("Pendulum Effect Style - Font Style") ??
            emptyCard.pendulumStyle.fontStyle
          ).toLowerCase();
          const effectBackground = (
            reader("Effect Style - Background") ??
            emptyCard.effectStyle.background
          ).toLowerCase();
          const pendulumEffectBackground = (
            reader("Pendulum Effect Style - Background") ??
            emptyCard.pendulumStyle.background
          ).toLowerCase();
          const effectMinLine = normalizeInt(
            reader("Effect Style - Min Line"),
            emptyCard.effectStyle.minLine
          );
          const pendulumEffectMinLine = normalizeInt(
            reader("Pendulum Effect Style - Min Line"),
            emptyCard.pendulumStyle.minLine
          );

          const emptyOpacity = getDefaultCardOpacity();
          const opacity: CardOpacity = {
            artBorder: normalizeBoolean(
              reader("Opacity - Art Border"),
              emptyOpacity.artBorder
            ),
            baseFill: reader("Opacity - Base Fill") ?? emptyOpacity.baseFill,
            body: normalizeInt(reader("Opacity - Body"), emptyOpacity.body),
            boundless: normalizeBoolean(
              reader("Opacity - Boundless"),
              emptyOpacity.boundless
            ),
            name: normalizeInt(reader("Opacity - Name"), emptyOpacity.name),
            nameBorder: normalizeBoolean(
              reader("Opacity - Name Border"),
              emptyOpacity.nameBorder
            ),
            pendulum: normalizeInt(
              reader("Opacity - Pendulum"),
              emptyOpacity.pendulum
            ),
          ...
          const artFit = normalizeBoolean(
            reader("Is Using Full Art"),
            emptyCard.artFit
          );
          const artCrop: Crop = {
            aspect: getArtCanvasCoordinate(
              isPendulum,
              opacity,
              undefined,
              pendulumSize
            ...
            height: normalizeFloat(
              reader("Art Crop - Height (%)"),
              emptyArtCrop.height
            ),
            width: normalizeFloat(
              reader("Art Crop - Width (%)"),
              emptyArtCrop.width
            ),
          ...
          const hasBackground = normalizeBoolean(
            reader("Has Background"),
            emptyCard.hasBackground
          );
          const background = reader("Background Link") ?? emptyCard.background;
          const backgroundType = (
            reader("Background Type") ?? emptyCard.backgroundType
          ).toLowerCase() as BackgroundType;
          const backgroundFit = normalizeBoolean(
            reader("Is Using Full Background"),
            emptyCard.backgroundFit
          );
          const backgroundCrop: Crop = {
            ...
            height: normalizeFloat(
              reader("Background Crop - Height (%)"),
              emptyBackgroundCrop.height
            ),
            width: normalizeFloat(
              reader("Background Crop - Width (%)"),
              emptyBackgroundCrop.width
            ),
            x: normalizeFloat(
              reader("Background Crop - X (%)"),
              emptyBackgroundCrop.x
            ),
            y: normalizeFloat(
              reader("Background Crop - Y (%)"),
              emptyBackgroundCrop.y
            ),
            unit: "%",
          };
          ...
          const nameStyleType = (
            reader("Name Style Type") ?? emptyCard.nameStyleType
          ).toLowerCase() as NameStyleType;
          const nameStyle: NameStyle = {
            embossPitch: normalizeInt(
              reader("Name Style - Emboss Pitch"),
              emptyNameStyle.embossPitch
            ),
            embossYaw: normalizeInt(
              reader("Name Style - Emboss Yaw"),
              emptyNameStyle.embossYaw
            ),
            embossThickness: normalizeInt(
              reader("Name Style - Emboss Thickness"),
              emptyNameStyle.embossThickness
            ),
            fillStyle: normalizeColor(
              reader("Name Style - Fill Style"),
              emptyNameStyle.fillStyle
            ),
            font: reader("Name Style - Font") ?? emptyNameStyle.font,
            gradientAngle: normalizeInt(
              reader("Name Style - Gradient Angle"),
              emptyNameStyle.gradientAngle
            ),
            gradientColor:
              reader("Name Style - Gradient Color") ??
              emptyNameStyle.gradientColor,
            hasEmboss: normalizeBoolean(
              reader("Name Style - Has Emboss"),
              emptyNameStyle.hasEmboss
            ),
            hasGradient: normalizeBoolean(
              reader("Name Style - Has Gradient"),
              emptyNameStyle.hasGradient
            ),
            hasOutline: normalizeBoolean(
              reader("Name Style - Has Outline"),
              emptyNameStyle.hasOutline
            ),
            hasShadow: normalizeBoolean(
              reader("Name Style - Has Shadow"),
              emptyNameStyle.hasShadow
            ),
            headTextFillStyle: normalizeColor(
              reader("Name Style - Headtext Fill Style"),
              emptyNameStyle.headTextFillStyle
            ),
            lineColor: normalizeColor(
              reader("Name Style - Line Color"),
              emptyNameStyle.lineColor
            ),
            lineOffsetX: normalizeInt(
              reader("Name Style - Line Offset X"),
              emptyNameStyle.lineOffsetX
            ),
            lineOffsetY: normalizeInt(
              reader("Name Style - Line Offset Y"),
              emptyNameStyle.lineOffsetY
            ),
            lineWidth: normalizeInt(
              reader("Name Style - Line Width"),
              emptyNameStyle.lineWidth
            ),
            pattern: reader("Name Style - Pattern") ?? emptyNameStyle.pattern,
            preset: (reader("Name Style - Preset") ??
              emptyNameStyle.preset) as PresetNameStyle,
            shadowBlur: normalizeInt(
              reader("Name Style - Shadow Blur"),
              emptyNameStyle.shadowBlur
            ),
            shadowColor: normalizeColor(
              reader("Name Style - Shadow Color"),
              emptyNameStyle.shadowColor
            ),
            shadowOffsetX: normalizeInt(
              reader("Name Style - Shadow Offset X"),
              emptyNameStyle.shadowOffsetX
            ),
            shadowOffsetY: normalizeInt(
              reader("Name Style - Shadow Offset Y"),
              emptyNameStyle.shadowOffsetY
            ),
          };

          const isLink = normalizeBoolean(
            reader("Is Link"),
            emptyCard.isLink ?? frame === "link"
          );
          const linkMap =
            [
              normalizeBoolean(reader("Link - Top Left Arrow"), false)
                ? "1"
                : "",
              normalizeBoolean(reader("Link - Top Arrow"), false) ? "2" : "",
              normalizeBoolean(reader("Link - Top Right Arrow"), false)
                ? "3"
                : "",
              normalizeBoolean(reader("Link - Left Arrow"), false) ? "4" : "",
              normalizeBoolean(reader("Link - Right Arrow"), false) ? "6" : "",
              normalizeBoolean(reader("Link - Bottom Left Arrow"), false)
                ? "7"
                : "",
              normalizeBoolean(reader("Link - Bottom Arrow"), false) ? "8" : "",
              normalizeBoolean(reader("Link - Bottom Right Arrow"), false)
                ? "9"
                : "",
            ].filter((entry) => entry !== "") ?? [];
          ...
          const statTextStyle = [
            normalizeBoolean(
              reader("Stat Style - Is Custom"),
              emptyTextStyle[0]
            ),
            normalizeColor(
              reader("Stat Style - Fill Color"),
              emptyTextStyle[1]
            ),
            normalizeBoolean(
              reader("Stat Style - Has Shadow"),
              emptyTextStyle[2]
            ),
            normalizeColor(
              reader("Stat Style - Shadow Color"),
              emptyTextStyle[3]
            ),
          ] as [boolean, string, boolean, string];
          const typeTextStyle = [
            normalizeBoolean(
              reader("Type Style - Is Custom"),
              emptyTextStyle[0]
            ),
            normalizeColor(
              reader("Type Style - Fill Color"),
              emptyTextStyle[1]
            ),
            normalizeBoolean(
              reader("Type Style - Has Shadow"),
              emptyTextStyle[2]
            ),
            normalizeColor(
              reader("Type Style - Shadow Color"),
              emptyTextStyle[3]
            ),
          ] as [boolean, string, boolean, string];
          const effectTextStyle = [
            normalizeBoolean(
              reader("Effect Style - Is Custom"),
              emptyTextStyle[0]
            ),
            normalizeColor(
              reader("Effect Style - Fill Color"),
              emptyTextStyle[1]
            ),
            normalizeBoolean(
              reader("Effect Style - Has Shadow"),
              emptyTextStyle[2]
            ),
            normalizeColor(
              reader("Effect Style - Shadow Color"),
              emptyTextStyle[3]
            ),
          ] as [boolean, string, boolean, string];
          const pendulumTextStyle = [
            normalizeBoolean(
              reader("Pendulum Effect Style - Is Custom"),
              emptyTextStyle[0]
            ),
            normalizeColor(
              reader("Pendulum Effect Style - Fill Color"),
              emptyTextStyle[1]
            ),
            normalizeBoolean(
              reader("Pendulum Effect Style - Has Shadow"),
              emptyTextStyle[2]
            ),
            normalizeColor(
              reader("Pendulum Effect Style - Shadow Color"),
              emptyTextStyle[3]
            ),
          ] as [boolean, string, boolean, string];
          const otherTextStyle = [
            normalizeBoolean(
              reader("Other Style - Is Custom"),
              emptyTextStyle[0]
            ),
            normalizeColor(
              reader("Other Style - Fill Color"),
              emptyTextStyle[1]
            ),
            normalizeBoolean(
              reader("Other Style - Has Shadow"),
              emptyTextStyle[2]
            ),
            normalizeColor(
              reader("Other Style - Shadow Color"),
              emptyTextStyle[3]
            ),
          ] as [boolean, string, boolean, string];
          let externalInfo = {};
          ...
          const baseFlag = (reader("Flag") ?? "")
            .split("|")
            .map(Number)
            .slice(0, FLAG_LENGTH) as CardFlag;
          const flag = getDefaultCardFlag().map((entry, index) => {
            if (typeof baseFlag[index] === "number") return baseFlag[index];
            return entry;
          }) as CardFlag;

          const baseDyeList = (reader("Dye List") ?? "")
            .split("|")
            .map(String)
            .slice(0, DYE_LIST_LENGTH) as FrameDyeList;
          const dyeList = getDefaultDyeList().map((entry, index) => {
            if (typeof baseDyeList[index] === "string")
              return baseDyeList[index];
            return entry;
          }) as FrameDyeList;

          const starList = (reader("Star List") ?? "")
            .split("|")
            .map(String)
            .slice(0, MAX_STAR_LENGTH);
