...
export const useCardExport = ({
  ...
  const addToCurrentBatch = useCallback(
    async (
      cardName: string,
      cardId: string,
      size: [number, number] = resolution
      ...
      document
        .querySelector("#export-canvas-guard")
        ?.classList.remove("guard-on");
      onDownloadComplete();
    },
    [
      addToBatch,
  ...
  const download = useCallback(
    (size: [number, number] = resolution) => {
      try {
        ...
        link.download = normalizedName ? `${normalizedName}.png` : "card.png";
    ...
    if (isInitializing === false) {
      ...
      (async () => {
        ...
        if (canvasRef) {
          ...
          document
            .getElementById("export-canvas")
            ?.classList.remove("js-export-available");
          document
            .getElementById("preview-canvas")
            ?.classList.remove("js-export-available");
          document
            .getElementById("export-canvas-guard")
            ?.setAttribute("style", "");
          document
            .getElementById("save-button-waiting")
          ...
          if (relevant) {
            ...
            if (relevant) {
              ...
              const condensedCard = JSON.stringify(
                compressCardData(normalizedCard)
              );
              if (typeof condensedCard === "string")
                insertUrlParam("data", condensedCard);

              document
                .getElementById("export-canvas")
                ?.classList.add("js-export-available");
              document
                .getElementById("preview-canvas")
                ?.classList.add("js-export-available");
              document
