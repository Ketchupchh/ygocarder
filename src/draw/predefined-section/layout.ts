...
export const baseDrawLinkArrowMap = async (
  ...
  await Promise.all<any>(
    [1, 2, 3, 4, 6, 7, 8, 9].map(async (entry) => {
      const { left, top, height, width } =
        ArrowPositionMap[positionType][entry - 1];
      ...
      await drawAssetWithSize(
        ctx,
        `${baseLink}-base${boundless ? "-full" : ""}.png`,
        ...coordinate
      );
...
export const baseDrawLinkMapFoil = async (
    ...
    if (withBorder) {
      ...
      await Promise.all<any>(
        [1, 2, 3, 4, 6, 7, 8, 9].map(async (entry) => {
          ...
          await drawAssetWithSize(
            ctx,
            `link/link-overlay-${entry}-${usedFoil}.png`,
            ...coordinate
          );
        })
      );
  ...
  if (willDyed) {
    ...
    const { canvas: dyedLinkMapFoilCanvas } = dyeCanvas(
      linkMapFoilCanvas,
      dyeColor
    );
...
export const getLayoutDrawFunction = ({
  ...
  pendulumEffectBackground: string;
  ...
  isPendulum: boolean;
  pendulumFrameTypeMap: {
    blue: "normal" | "scaleless";
    red: "normal" | "scaleless";
  };
  ...
  const artBoxY = 170,
    artBoxX = 60;
  const effectBoxY = 860,
    effectBoxX = 35;
  const backgroundEffectBoxY = effectBoxY + 25,
    backgroundEffectBoxX = effectBoxX + 19;
  const backgroundEffectBoxWidth = 705,
    backgroundEffectBoxHeight = 231;

  const hasFoil = foil !== "normal";
  const frameBorderType = isXyz || isSpeedSkill ? frame : "normal";
  const nameBorderTypeList =
    frame === "synchro"
      ? [
          "normal",
          "xyz",
          ...
          "xyz",
          "xyz",
        ] /** This produce more accurate effect */
      : [frameBorderType];
  ...
  const resultAPI = {
    ...
    drawFrame: async () => {
      ...
      const willReplaceFrame =
        hasBackground && backgroundCanvas && backgroundType === "frame";
      ...
      await drawAsset(
        bottomFrameCtx,
        `frame-pendulum/frame-pendulum-${bottomLeftFrame}.png`,
        0,
        0
      );
      const { canvas: dyedBottomFrameCanvas, ctx: dyedBottomFrameCtx } =
        dyeCanvas(bottomFrameCanvas, dyeList[2]);
      if (
        !willReplaceFrame &&
        (bottomLeftFrame !== bottomRightFrame ||
          dyeList[2] !== "" ||
          dyeList[3] !== "") &&
        dyedBottomFrameCtx
      ) {
      ...
      if (isPendulum) {
        dyedTopFrameCtx?.clearRect(
          artFrameX,
          artFrameY,
          artFrameWidth,
          artFrameHeight
        );
        dyedBottomFrameCtx?.clearRect(
          artFrameX,
          artFrameY,
          artFrameWidth,
          artFrameHeight
    ...
    drawCardArt: (externalCtx = ctx) => {
      ...
      externalCtx.drawImage(
        ...
        (globalScale * artWidth) / (imageWidth / imageHeight)
      );
    },
    drawBackground: async (externalCtx = ctx, hasArtBorder = false) => {
      ...
      if (backgroundType !== "frame") {
        ...
        externalCtx.drawImage(
          ...
          0,
          0,
          globalScale * cardWidth,
          globalScale * cardHeight
        );
      }
      const backgroundFinish = otherFinish[3] ?? "normal";
      if (backgroundFinish !== "normal" && hasArtBorder) {
        const loopBackgroundFinish = getFinishIterator(
          [backgroundFinish],
          ArtFinishMap
        );
        await loopBackgroundFinish(externalCtx, "art", async (type) =>
          drawAssetWithSize(
            externalCtx,
            `finish/art-finish-${type}.png`,
            globalScale * artFrameX,
            globalScale * artFrameY,
            globalScale * artFrameWidth,
            (globalScale * artFrameWidth) / artRatio
          )
        );
      }
    },
    drawAttribute: async () => {
      if (!ctx) return;
      ctx.scale(globalScale, globalScale);
      const { canvas: attributeCanvas, ctx: attributeCtx } = createCanvas(
        cardWidth * globalScale,
        (attributeY + attributeSize) * globalScale
      );
      await drawAsset(
        attributeCtx,
        `attribute/attr-${
          RegionMap[region].fileKey
        }-${attribute.toLowerCase()}.png`,
        attributeX,
        attributeY
      );
      const attributeFinish = otherFinish[0] ?? "normal";
      if (attributeFinish !== "normal") {
        const loopAttributeFinish = getFinishIterator(
          [attributeFinish],
          ArtFinishMap
        );
        const { canvas: attributeFinishCanvas, ctx: attributeFinishCtx } =
        ...
        await loopAttributeFinish(
          ...
          async (finishType) => {
            return await drawAsset(
              ...
              (cardWidth - finishTypeCardWidth) / 2,
              stickerSize
            );
          }
        );
    ...
    drawStar: async ({
      ...
      const normalizedCardIcon =
        cardIcon === "auto" ? getCardIconFromFrame(frame) : cardIcon;

      if (!ctx) return;
      ctx.scale(globalScale, globalScale);
      const starFinish = otherFinish[1] ?? "normal";
      const loopStarFinish =
        starFinish !== "normal"
          ? getFinishIterator([starFinish], ArtFinishMap)
          : undefined;
      await drawStarContent({
        ...
        onStarDraw: async (coordinate) => {
          return normalizedCardIcon === "st"
            ? Promise.resolve()
            : await loopFinish(ctx, "star", async (type) =>
                drawAsset(ctx, `finish/finish-${type}-star.png`, ...coordinate)
              );
        },
        loopStarFinish,
      });
    ...
    drawPendulumScaleIcon: async () => {
      const { canvas: pendulumIconCanvas, ctx: pendulumIconCtx } = createCanvas(
        pendulumIconFrameWidth,
        pendulumIconFrameHeight
      );
      ...
      await drawAsset(
        pendulumIconCtx,
        `frame-pendulum/pendulum-scale-${pendulumSize}.png`,
        0,
        0
      );

      if (pendulumFrameTypeMap.blue === "scaleless") {
        pendulumIconCtx.clearRect(
          0,
          0,
          pendulumIconFrameWidth / 2,
          pendulumIconFrameHeight
        );
      }
    ...
    drawNameBackground: async () => {
      ...
      ctx.scale(globalScale, globalScale);
      const { ctx: nameBackgroundCtx, canvas: nameBackgroundCanvas } =
        createCanvas(cardWidth, topToPendulumStructure);
      await drawAsset(
        nameBackgroundCtx,
        `background/background-name-${topLeftFrame}.png`,
        0,
        0
      );
      const { canvas: dyedLeftNameCanvas, ctx: dyedLeftNameCtx } = dyeCanvas(
        nameBackgroundCanvas,
        dyeList[0]
      );
      if (
        topLeftFrame !== topRightFrame ||
    ...
    drawEffectBackground: async (withPendulum = false) => {
      ...
      const { ctx: effectBackgroundCtx, canvas: effectBackgroundCanvas } =
        createCanvas();
      if (withPendulum) {
        await drawAssetWithSize(
          effectBackgroundCtx,
          `background/background-text-${resolvedEffectBackground}.png`,
          backgroundEffectBoxX,
          backgroundEffectBoxY + effectBoxOffsetY,
          backgroundEffectBoxWidth,
          backgroundEffectBoxHeight,
          0,
          0 + effectBoxOffsetY,
          backgroundEffectBoxWidth,
          backgroundEffectBoxHeight
        ...
        await drawAsset(
          ...
          `background/background-text-${resolvedEffectBackground}.png`,
          backgroundEffectBoxX,
          backgroundEffectBoxY
        );
      }
      const dyedEffectBackgroundCanvas = dyeCanvas(
        effectBackgroundCanvas,
        dyeList[4]
      ).canvas;
      ...
      if (withPendulum) {
        const {
          ctx: pendulumEffectBackgroundCtx,
          canvas: pendulumEffectBackgroundCanvas,
        } = createCanvas();
        const {
          exceptionFrameType = frameType,
          exceptionPendulumBoxOffsetHeight = 0,
        } =
          PendulumSizeMapException[pendulumSize][
            resolvedPendulumEffectBackground
          ] ?? {};
        await drawAssetWithSize(
          pendulumEffectBackgroundCtx,
          `background/background-${exceptionFrameType}-${resolvedPendulumEffectBackground}.png`,
          pendulumBoxX,
          pendulumBoxY + pendulumBoxOffsetY,
          pendulumBoxWidth,
          pendulumBoxHeight,
          0,
          pendulumBoxOffsetY + exceptionPendulumBoxOffsetHeight,
          pendulumBoxWidth,
          pendulumBoxHeight + exceptionPendulumBoxOffsetHeight
        );
        const dyedPendulumEffectBackgroundCanvas = dyeCanvas(
          pendulumEffectBackgroundCanvas,
          dyeList[5]
        ).canvas;
    ...
    drawNameBorder: async () => {
      if (nameBorder && ctx) {
        ...
        for (let index = 0; index < nameBorderTypeList.length; index++) {
          await drawAsset(
            ctx,
            `frame/name-border-${nameBorderTypeList[index]}.png`,
            0,
            0
          );
        }
    ...
    drawArtBorder: async () => {
      if (artBorder && ctx) {
        ...
        if (isXyz || isSpeedSkill) {
          ...
          isLink ||
          ["zarc", "hamon", "uria", "raviel"].includes(frame)
        ) {
    ...
    drawPendulumBorder: async (
      ...
      const usedFoil =
        validDyeColor && willDye && foilType === "normal"
          ? "platinum"
          : foilType;
      /** We create a new canvas for easier manipulation. */
      const { canvas: pendulumBorderCanvas, ctx: pendulumBorderCtx } =
        createCanvas();
      await drawAssetWithSize(
        pendulumBorderCtx,
      ...
      if (willDye && validDyeColor) {
        const { canvas: dyedCardBorderFoilCanvas } = dyeCanvas(
          pendulumBorderCanvas,
          dyeList[6]
        );
      ...
      if (artBorder) {
         ...
         */
        pendulumBorderCtx?.clearRect(
          0,
          0,
          cardWidth,
          artlessFrameY + topToPendulumStructureFrame
        );
        if (validDyeColor) {
          /** Because we do not have "normal" foil, we use platinum as a base to dye. */
          const usedFoil = foilType === "normal" ? "platinum" : foilType;
          const {
            ctx: pendulumBorderFoilCtx,
            canvas: pendulumBorderFoilCanvas,
          } = createCanvas();
          await drawAsset(
          ...
          const { canvas: dyedPendulumBorderFoilCanvas } = dyeCanvas(
            pendulumBorderFoilCanvas,
            dyeList[6]
          );
    ...
    drawEffectBorder: async () => {
      ...
      if (!hasFoil && bottomLeftFrame === "speed-skill") {
        await drawAsset(
          ctx,
          "frame/effect-border-speed-skill.png",
          effectBoxX,
          effectBoxY
        );
      } else {
    ...
    drawCardBorder: async () => {
      ...
      if (HexColorRegex.test(dyeList[6])) {
        ...
        const { ctx: cardBorderFoilCtx, canvas: cardBorderFoilCanvas } =
          createCanvas();
        await drawAsset(
          cardBorderFoilCtx,
          `frame/card-border-${usedFoil}.png`,
          0,
          0
        );
        const { canvas: dyedCardBorderFoilCanvas } = dyeCanvas(
          cardBorderFoilCanvas,
          dyeList[6]
        );
        ...
        await drawAsset(
          ctx,
          `frame/card-border${hasFoil ? `-${foil}` : ""}.png`,
          0,
          0
        );
      }
    ...
    drawArtBorderFoil: async () => {
      ...
      if (artBorder) {
        if (HexColorRegex.test(dyeList[6])) {
          /** Because we do not have "normal" foil, we use platinum as a base to dye. */
          const usedFoil = foil === "normal" ? "platinum" : foil;
          const { ctx: artBorderFoilCtx, canvas: artBorderFoilCanvas } =
            createCanvas();
          await drawAsset(
            artBorderFoilCtx,
            `frame/art-border-${usedFoil}.png`,
            artBoxX,
            artBoxY
          );
          const { canvas: dyedArtBorderFoilCanvas } = dyeCanvas(
            artBorderFoilCanvas,
            dyeList[6]
          );
    ...
    drawEffectBorderFoil: async () => {
      ...
      if (HexColorRegex.test(dyeList[6])) {
        /** Because we do not have "normal" foil, we use platinum as a base to dye. */
        const usedFoil = foil === "normal" ? "platinum" : foil;
        const { ctx: effectBorderFoilCtx, canvas: effectBorderFoilCanvas } =
          createCanvas();
        await drawAsset(
          effectBorderFoilCtx,
          `frame/effect-border-${usedFoil}.png`,
          effectBoxX,
          effectBoxY
        );
        const { canvas: dyedEffectBorderFoilCanvas } = dyeCanvas(
          effectBorderFoilCanvas,
          dyeList[6]
        );
    ...
    drawNameFinish: async () => {
      if ((artBorder || nameBorder) && ctx) {
        ctx.scale(globalScale, globalScale);
        await loopFinish(ctx, "name", async (type) => {
    ...
    drawAttributeFinish: async () => {
      if (attribute !== NO_ATTRIBUTE && ctx) {
        ctx.scale(globalScale, globalScale);
        await loopFinish(ctx, "attribute", async (type) =>
          drawAsset(ctx, `finish/finish-${type}-attribute.png`, 678, 55)
        );
        ctx.resetTransform();
      }
    },
    drawArtFinish: async (artCtx = ctx) => {
      ...
      await loopArtFinish(
        ...
        async (finishType, pendulumSuffix) => {
          return await drawAssetWithSize(
            artCtx,
            `finish/art-finish-${finishType}${
              isPendulum ? pendulumSuffix : ""
            }.png`,
            artFinishX,
            artFinishY,
            artFrameWidth,
            artFrameWidth / artRatio
          );
        },
        pendulumSize
      );
     ...
     */
    drawArtOverlayFinish: async (artCtx = ctx) => {
      if (!artCtx) return;
      artCtx.scale(globalScale, globalScale);
      await loopFinish(artCtx, "art-overlay", async (type) => {
        return drawAsset(
          artCtx,
          "finish/finish" +
            `-${type}` +
    ...
    drawArtBorderFinish: async () => {
      if (artBorder && ctx) {
        ctx.scale(globalScale, globalScale);
        await loopFinish(ctx, "art-border", async (finishType) =>
          drawAsset(ctx, `finish/finish-${finishType}-art-border.png`, 0, 0)
        );
        ctx.resetTransform();
      }
    },
    drawPendulumArtBorderFinish: async () => {
      ...
      ctx.scale(globalScale, globalScale);
      if (artBorder)
        await loopFinish(ctx, "art-border-pendulum", (finishType) =>
          drawAsset(
            ctx,
    ...
    drawBorderPendulumFinish: async () => {
      if (!ctx) return;
      ctx.scale(globalScale, globalScale);
      await loopFinish(ctx, "border-pendulum", async (finishType) => {
    ...
    drawFrameFinish: async () => {
      if (!ctx) return;
      ctx.scale(globalScale, globalScale);
      await loopFinish(ctx, "frame", async (type) => {
        return drawAsset(
          ctx,
          `finish/finish-${type}-frame${
            isPendulum ? `-pendulum-${pendulumSize}` : ""
          }.png`,
          0,
          0
    ...
    drawFrameBackgroundFinish: async () => {
      if (!ctx) return;
      ctx.scale(globalScale, globalScale);
      await loopFinish(ctx, "frame-background", async (type) => {
        return drawAsset(
          ctx,
          `finish/finish-${type}-frame-background${
            isPendulum ? "-pendulum" : ""
          }.png`,
          0,
          0
    ...
    drawOverlayFinish: async () => {
      if (!ctx) return;
      ctx.scale(globalScale, globalScale);
      await loopFinish(ctx, "overlay", async (overlayType) =>
        drawAsset(ctx, `finish/finish-${overlayType}-overlay.png`, 0, 0)
      );
      ctx.resetTransform();
    },
    drawCardBorderFinish: async () => {
      if (!ctx) return;
      ctx.scale(globalScale, globalScale);
      await loopFinish(ctx, "card-border", async (type) =>
        drawAsset(ctx, `finish/finish-${type}-card-border.png`, 0, 0)
      );
      ctx.resetTransform();
    },
  };
