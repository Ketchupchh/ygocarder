...
const { width: CanvasWidth, finishTypeCardWidth, stickerSize } = CanvasConst;
...
export const drawStarContent = async ({
  ...
  const normalizedStarList = Array.isArray(starList) ? starList : [];
  const starWidth = 50;
  const startSpacing = 4;
  let normalizedStarCount =
    cardIcon === "custom"
      ...
      : 0;
  let totalWidth =
    starWidth * normalizedStarCount + startSpacing * (normalizedStarCount - 1);
  const baseline = 145;

  let alignment = "right";
  if (["rank", "negative-level"].includes(cardIcon)) alignment = "left";
  alignment = starAlignment === "auto" ? alignment : starAlignment;
  ...
  const { canvas: starCanvas, ctx: starCtx } = createCanvas(
    CanvasWidth * globalScale,
    (baseline + starWidth) * globalScale
  );
  await Promise.all(
    [...Array(normalizedStarCount)].map(async (_, index) => {
      ...
      const cardIconName =
        cardIcon === "custom" ? normalizedStarList[cardIconIndex] : cardIcon;
  ...
  if (loopStarFinish) {
    const { canvas: starFinishCanvas, ctx: starFinishCtx } = createCanvas(
      CanvasWidth,
      baseline + starWidth
    );
    starFinishCtx.drawImage(starCanvas, 0, 0);
    await loopStarFinish(starFinishCtx, "art", async (finishType) => {
      return await drawAsset(
        starFinishCtx,
        `finish/finish-typeart-${finishType}.png`,
        (CanvasWidth - finishTypeCardWidth) / 2,
        stickerSize
      );
    });
...
export const drawLimitedEditionMark = async ({
  ...
  const coordinate: [number, number, number, number] =
    !isLegacyCard || isPendulum
...
export const drawLinkRatingText = async (
  ...
  const baseline = isNaN(Number(rating)) ? 1106 : 1105;
