...
export const drawTypeAbilityText = ({
  ...
  const {
    ...
    TypeAbilityCoordinateMap[format]?.[size] ??
      TypeAbilityCoordinateMap["tcg"]["medium"],
    globalScale
  );
  const scaledBaseFontData = scaleFontData(
    getTypeAbilityFontData()[format],
    globalScale
  );
  const fontData = {
    ...scaledBaseFontData,
  ...
  ctx.textAlign = "left";
  const normalizedText = normalizeCardText(value, format, {
    multiline: false,
    furiganaHelper,
  });
  const internalEffectiveMedian = condense((median) => {
  ...
  const result = drawLine({
    ...
    yRatio,
    trueEdge:
      edgeAlignment === "left" ? trueEdge : trueEdge - actualLineWidth * xRatio,
    trueBaseline,
    textData,
    format,
    globalScale,
...
export const drawTypeAbility = async ({
  ...
  const typeAbilityWithIcon =
    typeAbility + (willDrawIcon ? ST_ICON_SYMBOL : "");
  ...
  const defaultFillStyle =
    checkLightFrame(frame) && !checkSpeedSkill({ frame }) && size === "large"
      ? "#ffffff"
      : "#000000";
  ...
  if (willDrawIcon) {
    ...
    const { canvas: iconCanvas, ctx: iconCtx } = createCanvas(
      ...
      (baseline + iconWidth) * globalScale
    );
    ...
    if (loopIconFinish) {
      const { canvas: iconFinishCanvas, ctx: iconFinishCtx } = createCanvas(
        CanvasWidth * globalScale,
        (baseline + iconWidth) * globalScale
      );
      iconFinishCtx.drawImage(iconCanvas, 0, 0);
      await loopIconFinish(iconFinishCtx, "art", async (finishType) => {
