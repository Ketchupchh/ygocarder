...
export const drawCreatorText = ({
  ...
  const resetTextStyle = setTextStyle({
    ...
    shadowColor: hasShadow ? (lightFooter ? "#000000" : "#ffffff") : "#000000",
    y: 0,
    x: 0,
    blur: hasShadow ? 3 : 0,
    globalScale,
  ...
  for (let fontLevel = 0; fontLevel < fontData.fontList.length; fontLevel++) {
    ...
    ctx.textAlign = "left";
    internalEffectiveMedian = condense((median) => {
  ...
  const result = drawLine({
    ...
    yRatio,
    trueEdge:
      alignment === "left" ? trueEdge : trueEdge - actualLineWidth * xRatio,
    trueBaseline:
      trueBaseline +
      (fontSizeData.offsetY ?? scaledDefaultFontSizeData.offsetY) +
      baselineOffset * globalScale,
    textData,
    format,
    globalScale,
