...
export const drawEffect = ({
  ...
  const { lineList, fullLineListOption, effectFlavorCondition } = splitEffect(
    normalizedContent,
    isNormal
  );
  ...
  while (sizeLevel <= fontList.length && sizeLevel >= 0) {
    const requireDynamicSize = sizeLevel === fontList.length ? true : false;
    ...
    const {
      ...
      sizeList[appliedSizeLevel] ?? sizeList[sizeList.length - 1],
      globalScale
    );
    const width =
      isNormal && format === "tcg"
        ? trueWidthStart - 2 * globalScale
        : trueWidthStart;

    const useDynamicSize =
      requireDynamicSize && typeof trueHeightCap === "number";
    const dynamicFontData = useDynamicSize
      ? injectDynamicFont(scaledFontData, {
          heightCap: trueHeightCap,
          lineCount: effectiveLineCount,
        })
      : scaledFontData;
    const dynamicSizeLevel = useDynamicSize ? sizeLevel : appliedSizeLevel;
    const { font: dynamicFont, fontList: dynamicFontList } = dynamicFontData;
    const fontSizeData = useDynamicSize
      ? dynamicFontList[dynamicSizeLevel]
      : fontList[appliedSizeLevel];
    const { fontSize, lineHeight, lineCount } = fontSizeData;
    ...
    const effectiveMedian =
      lineList.length > lineCount && typeof trueHeightCap === "number"
    ...
    const tolerantValue =
      tolerancePerSentence[`${lineList.length}`] ?? tolerancePerSentence["3"];
    if (
      ...
      lineListWithRatio.forEach(
        ({ line: precalculatedLine, effectiveMedian, isLast }) => {
          if (precalculatedLine === FULL_LINE_PLACEHOLDER) {
            ...
            const flavorFontData = scaleFontData(
              EffectFontData[fontDataKey],
              globalScale
            );
            const dynamicFlavorFontData = useDynamicSize
              ? injectDynamicFont(flavorFontData, {
                  heightCap: trueHeightCap,
                  lineCount: Math.max(minLine, lineListWithRatio.length),
                })
              : flavorFontData;
