...
export const splitEffect = (effect: string, isNormal = false) => {
  ...
  const wholeLineRegex = new RegExp(
    [lineRegexSource, fullLineRegexSource].join("|")
  );
  ...
  while (effectText.length > 0 && tryCnt <= 100) {
    ...
    if (lineContent && lineReplacement) {
      ...
      if (lineReplacement) {
        ...
        lineList.push(
          ...lineContent
            .replaceAll(NB_LINE_OPEN, "[")
            .replaceAll(NB_LINE_CLOSE, "]")
            .replaceAll(NB_FULL_LINE_OPEN, "[[")
            .replaceAll(NB_FULL_LINE_CLOSE, "]]")
            .split("\n")
        );
        effectText = effectText.replace(lineReplacement, "");
      }
...
export const normalizeCardText = (
  ...
  const textAfterSplitBlockRow = multiline
    ? textAfterJoinRow
        .replace(
          /^\[\[([\w\W]*?)\]\](?=(\n|$))/gm,
          `${NB_FULL_LINE_OPEN}$1${NB_FULL_LINE_CLOSE}`
        )
        .replace(
          /^\[([\w\W]*?)\](?=(\n|$))/gm,
          `${NB_LINE_OPEN}$1${NB_LINE_CLOSE}`
        )
    : textAfterJoinRow;
  ...
  const textAfterDetectBlockWord = textAfterSwapLetter
    ...
    .replaceAll(new RegExp(UNCOMPRESSED_SOURCE, "g"), (m) =>
      m
        .replaceAll("{{", NB_UNCOMPRESSED_START)
        .replaceAll("}}", NB_UNCOMPRESSED_END)
    )
    /** Convert ruby syntax into internal control characters */
    .replaceAll(
      new RegExp(OCG_RUBY_SOURCE, "g"),
      (m) => `${NB_WORD_OPEN}${m}${NB_WORD_CLOSE}`
    );

  /** Apply dictionary */
  const textAfterDictionaryMatch =
    format === "tcg" || furiganaHelper === false
      ? textAfterDetectBlockWord
      : textAfterDetectBlockWord.replaceAll(
          new RegExp(OCG_KEYWORD_SOURCE, "g"),
          (m) => {
            const keywordSubtitue =
              ocgKeywordDataMap[m][dictionaryType] ??
              ocgKeywordDataMap[m].rubyForm;
  ...
  const textAfterProcessing = textAfterDictionaryMatch
    .replaceAll(
      new RegExp(WHOLE_WORD_SOURCE, "g"),
      (m) => `${NB_WORD_OPEN}${m}${NB_WORD_CLOSE}`
    )
    .replaceAll(
      new RegExp(NOT_END_OF_LINE_SOURCE, "g"),
      (m) => `${NB_WORD_OPEN}${m}${NB_WORD_CLOSE}`
    )
    .replaceAll(
      new RegExp(NOT_START_OF_LINE_SOURCE, "g"),
      (m) => `${NB_WORD_OPEN}${m}${NB_WORD_CLOSE}`
    )
    .replaceAll(
      new RegExp(NOT_SPLIT_SOURCE, "g"),
      (m) => `${NB_WORD_OPEN}${m}${NB_WORD_CLOSE}`
    )
    .replaceAll(
      new RegExp(OCG_BULLET_SOURCE, "g"),
      (m) => `${NB_WORD_OPEN}${m}${NB_WORD_CLOSE}`
    );
  ...
  const finalizedText = textAfterNormalizeBlockWord
    .join("")
    .replaceAll(
      `${NB_FULL_LINE_CLOSE}${NB_WORD_CLOSE}`,
      `${NB_WORD_CLOSE}${NB_FULL_LINE_CLOSE}`
    )
    .replaceAll(
      `${NB_LINE_CLOSE}${NB_WORD_CLOSE}`,
      `${NB_WORD_CLOSE}${NB_LINE_CLOSE}`
    )
    .replaceAll(
      `${NB_WORD_OPEN}${NB_FULL_LINE_OPEN}`,
      `${NB_FULL_LINE_OPEN}${NB_WORD_OPEN}`
    )
    .replaceAll(
      `${NB_WORD_OPEN}${NB_LINE_OPEN}`,
      `${NB_LINE_OPEN}${NB_WORD_OPEN}`
    )
    .replaceAll(
      `${NB_UNCOMPRESSED_START}${NB_WORD_CLOSE}`,
      `${NB_WORD_CLOSE}${NB_UNCOMPRESSED_START}`
    )
    .replaceAll(
      `${NB_WORD_OPEN}${NB_UNCOMPRESSED_END}`,
      `${NB_UNCOMPRESSED_END}${NB_WORD_OPEN}`
    )
