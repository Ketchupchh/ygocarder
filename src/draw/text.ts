...
export const drawLine = ({
  ...
  const { currentFont, fontData, fontLevel } = textData;
  ...
  const scaledDefaultFontSizeData = scaleFontSizeData(
    DefaultFontSizeData,
    globalScale
  );
  const {
  ...
  const textWorker = getTextWorker(
    ctx,
    fontData,
    fontSizeData,
    currentFont,
   ...
   */
  for (
    let tokenCnt = 0, xRatio = baseXRatio;
    ...
    const fragmentList = token
      .split(FragmentSplitRegex)
    ...
    const {
      ...
      token,
      nextToken,
      previousTokenGap: previousTokenGap / xRatio,
    ...
    const indent =
      tokenCnt === 0
        ? (leftGap > 0
            ? Math.min(
                MAX_LINE_REVERSE_INDENT * globalScale,
                leftGap * xRatio
              ) * -1
            : 0) +
          (OCGAlphabetRegex.test(leftMostLetter)
            ? START_OF_LINE_ALPHABET_OFFSET * globalScale
            : 0)
        : 0;
    ...
    for (
      let fragmentCnt = 0;
      fragmentCnt < fragmentList.length;
      fragmentCnt++
    ) {
      ...
      /** We do not actually draw S/T Icon here, but we record its position and leave a suitable space for it. */
        iconPositionList.push({
          edge: fragmentEdge,
          size: iconSymbolWidth,
          baseline,
        });
      ...
      /** Fragment with overhead text. */
        ...
        const { headTextWidth, halfGap: baseHalfGap } = analyzeHeadText({
          headText,
          headTextLetterWidth,
          footText,
          ...
          xRatio:
            headTextOverflow === "condense" && footText.length > 1
        ...
        if (
          totalVacantSpace > 0 &&
          nextLeftGap >= -2 * globalScale &&
          isNextTokenOffsetable
        ) {
          rebalancedSpace =
            (totalVacantSpace + Math.max(nextLeftGap, 0) * 2) / 3;
        ...
        fragmentEdge +=
          Math.max(footTextWidth * xRatio, headTextWidth) -
          lostLeftWidth +
          spaceWidth;
      ...
      /** Draw "whole words". */
        const normalizedWordSpacingRatio = wordLetterSpacing
          ...
          : letterSpacingRatio;
        ctx.letterSpacing = `${
          (normalizedWordSpacingRatio - 1) *
          currentFont.getFontInfo().sizeAsNumber
        }px`;
        ...
        while (remainFragment !== "") {
          ...
          if (SquareBracketLetterRegex.test(currentLetter)) {
            ...
            actualLetterWidth =
              ctx.measureText(remainFragment).width -
              ctx.measureText(nextRemainFragment).width;
            drawLetter({
            ...
            const letterOffset =
              actualLetterWidth >
              ctx.measureText(currentLetter).width * capitalLetterRatio
            ...
            VietnameseDiacriticLetterRegex.test(currentLetter) &&
            fontStyle === "tcg"
          ) {
            applyVietnameseFont();
            actualLetterWidth =
            ...
            TCGSymbolLetterRegex.test(currentLetter) &&
            fontStyle === "tcg"
          ) {
            const { fontSize } = applySymbolFont(
              letterOffsetMap[currentLetter]?.ratio
            );
            actualLetterWidth =
              ctx.measureText(remainFragment).width -
              ctx.measureText(nextRemainFragment).width;
            drawLetter({
              ...drawLetterofWordParameter,
              baseline:
                drawLetterofWordParameter.baseline +
                fontSize * (letterOffsetMap[fragment]?.baseline ?? 0),
            });
            stopApplySymbolFont();
          } else {
            actualLetterWidth =
      ...
      /** Some specific letter ("Evil★Twin's Trouble Sunny" TCG) requires different font. */
        ...
        const letterWidth =
          ctx.measureText(letter).width * letterSpacingRatio * xRatio;
      ...
      /** All other fragments */
        ...
        const letterWidth =
          letterMetric.boundWidth * letterSpacingRatio * xRatio;
        const leftGap = Math.max(defaultGap, letterWidth * gapRatio);
        const rightGap = leftGap;
        const lostLeftWidth = getLostLeftWidth(currentRightGap, leftGap);
