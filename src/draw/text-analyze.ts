...
}: // debug = false,
{
  ...
  const letterSpacing =
    _letterSpacing ?? scaledDefaultFontSizeData.letterSpacing;
  ...
  const token = rawToken.replaceAll(
    new RegExp(NON_BREAKABLE_SYMBOL_SOURCE, "g"),
    ""
  );
  ...
  for (let cnt = 0; cnt < fragmentList.length; cnt++) {
    ...
    /** Special symbol in TCG card ("Evil☆Twin") may use different font */
      ...
      const fragmentWidth =
        ctx.measureText(fragment).width * letterSpacingRatio;
      stopApplySymbolFont();
      ...
      const { boundWidth } = getLetterWidth({
        ctx,
        letter: fragment,
        isLastOfLine,
        fontStyle,
...
export const analyzeLine = ({
  ...
  for (let cnt = 0, xRatio = baseXRatio; cnt < tokenList.length; cnt++) {
    ...
    const indent =
      (cnt === 0 && leftGap > 0
        ? Math.min(MAX_LINE_REVERSE_INDENT * globalScale, leftGap * xRatio) * -1
        : 0) +
      (cnt === 0 && OCGAlphabetRegex.test(leftMostLetter)
        ? START_OF_LINE_ALPHABET_OFFSET * globalScale * xRatio
        : 0);

    currentGap = rightGap * xRatio;
    totalContentWidth += totalWidth * xRatio + indent;
    lineSpaceCount +=
      spaceCount - (spaceAtEnd && nextToken === undefined ? 1 : 0);
  }
  const expectedSpaceWidth =
    lineSpaceCount > 0 ? (width - totalContentWidth) / lineSpaceCount : 0;
  const spaceWidth = isLast
    ? format === "tcg"
      ? expectedSpaceWidth > 1.5 * globalScale
        ? 0
        : expectedSpaceWidth
      : expectedSpaceWidth > 3.65 * globalScale
      ? 0
      : expectedSpaceWidth
    : expectedSpaceWidth;
