...
export function distanceTransform(
  ...
  var phase2ResultAllocation = pool.mallocDouble(size),
    phase2ResultArray = ndarray(
      phase2ResultAllocation,
      shape.slice(0),
      stride.slice(0),
      0
    ),
  ...
  var phase1ResultAllocation = pool.mallocDouble(size),
    phase1ResultArray = ndarray(
      phase1ResultAllocation,
      shape.slice(0),
      stride.slice(0),
      0
    );
  for (let rowIndex = 0; rowIndex < array.shape[0]; rowIndex++) {
    for (let columnIndex = 0; columnIndex < array.shape[1]; columnIndex++) {
      phase1ResultArray.set(
        rowIndex,
        columnIndex,
        array.get(rowIndex, columnIndex) ? 0 : infinityHeight
      );
    }
  }
  //Perform first phase of distance transform
  phase1(
    phase1ResultArray.data,
    (size / shape[dimensionCount - 1]) | 0,
    shape[dimensionCount - 1] | 0
  );
  ...
  for (index = dimensionCount - 1; index > 0; --index) {
    ...
    if (!isFinite(exponent)) {
      phase2_inf(
        phase2ResultArray.data,
        (size / currentDimensionSize) | 0,
        currentDimensionSize | 0,
        s_q,
        t_q
      );
    } else if (exponent === 1) {
