...
import {
  getDefaultHeightToNormalMapOption,
  HeightToNormalMap,
  SHADER_FILTER_TYPE,
} from "./shader";
...
export const applyEmboss = ({
  ...
  const computedLightAngleVec: [number, number, number] = lightAngleVec
    ...
    : [1, 0, 0];
  const angleVecMagnitude = Math.sqrt(
    computedLightAngleVec[0] ** 2 +
      computedLightAngleVec[1] ** 2 +
      computedLightAngleVec[2] ** 2
  );
  ...
  try {
    ...
    const dtImageData = distanceTransform(
      ndarray(validPixelList, [height, width])
    );
    const row = dtImageData.shape[0],
      column = dtImageData.shape[1];
    ...
    heightmapCanvas
      .getContext("2d")
      .putImageData(new ImageData(nextDtBitmap, width, height), 0, 0);
    ...
    const heightToNormalMapConverter = new HeightToNormalMap(
      getDefaultHeightToNormalMapOption(),
      normalMapCanvas
    );
    ...
    softenNormalMapContext.drawImage(normalMapCanvas, 0, 0, width, height);
    ...
    for (let pixelCnt = 0; pixelCnt < data.length; pixelCnt += 4) {
      ...
      const baseIntensity =
        nx * normalizedLightAngleVec[0] +
        ny * normalizedLightAngleVec[1] +
        nz * normalizedLightAngleVec[2];
      const enhancedIntensity =
        (baseIntensity / 2) * (Math.abs(minIntensity) + Math.abs(maxIntensity));
      const intensity = Math.max(
        minIntensity,
        Math.min(enhancedIntensity, maxIntensity)
      );

      /** Step 3b: We apply the original alpha channel into the result pixel to soften it, removing rough edges. */
      resultData[pixelCnt] = Math.round(
        data[pixelCnt] + lightColorVec[0] * intensity
      );
      resultData[pixelCnt + 1] = Math.round(
        data[pixelCnt + 1] + lightColorVec[1] * intensity
      );
      resultData[pixelCnt + 2] = Math.round(
        data[pixelCnt + 2] + lightColorVec[2] * intensity
      );
