...
type HeightToNormalMapOption = Partial<
  ReturnType<typeof getDefaultHeightToNormalMapOption>
>;
...
export class HeightToNormalMap {
  ...
  constructor(
    option: HeightToNormalMapOption,
  ...
  setStrengthAndLevel(strength: number, level: number) {
    this.normalMapUniform.dz.value =
      (1.0 / strength) * (1.0 + Math.pow(2.0, level)); //  eslint-disable-line
  }
