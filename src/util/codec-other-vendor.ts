...
const reverseCardIconMap = Object.entries(cardIconMap).reduce<
...
const reverseBoxSizeMap = Object.entries(boxSizeMap).reduce<
  Record<string, PendulumSize>
>((acc, cur) => {
  const [vendorValue, ourValue] = cur;
...
const reverseAttributeMap = Object.entries(attributeMap).reduce<
  Record<string, string>
>((acc, cur) => {
  const [vendorValue, ourValue] = cur;
  acc[ourValue] = vendorValue;
...
const reverseFrameMap = Object.entries(frameMap).reduce<Record<string, string>>(
  (acc, cur) => {
    const [vendorValue, ourValue] = cur;
    acc[ourValue] = vendorValue;
...
const reverseRarityMap: Record<
  string,
  { key: string; text?: string; artFinish?: string; finish?: string[] }
> = {
  common: {
    key: "",
  },
...
const rarityMap = Object.entries(reverseRarityMap).reduce<
  Record<string, string>
>((acc, cur) => {
...
export const cardMakerToYgoCarderData = (
  card: OtherMakerCard
): { result: Card; isPartial: boolean } => {
  const {
    name,
    atk,
    def,
  ...
  const normalizedRarity = rarity
    ? reverseRarityMap[rarity.toLowerCase()]
    : undefined;
  ...
  let isPartial =
    ...
    normalizedFrame === undefined ||
    normalizedRarity === undefined;
  const { finish, artFinish, text } = normalizedRarity ?? {};
  ...
  const result: Card = {
    ...
    nameStyle: namePreset ? namePreset.value : getDefaultNameStyle(),
    atk,
    def,
    ...
    star:
      `${levelAsNumber}` === level && levelAsNumber <= 13
        ? levelAsNumber
        : level,
