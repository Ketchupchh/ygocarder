...
export const createFontGetter = (props?: {
  ...
  let size: `${number}px` =
    typeof defaultSize === "number" ? `${defaultSize}px` : defaultSize;
  let sizeAsNumber =
    typeof defaultSize === "number"
      ? defaultSize
      : parseFloat(defaultSize.replaceAll("px", ""));
  ...
  return {
    ...
    setSize(nextSize) {
      const calculatedSize =
        typeof nextSize === "function" ? nextSize(sizeAsNumber) : nextSize;
      size =
...
export const injectDynamicFont = (
  fontData: FontData,
  dynamicFontOption: Parameters<typeof getDynamicFont>[0]
): FontData => {
  return {
    ...fontData,
    fontList: [...fontData.fontList, getDynamicFont(dynamicFontOption)],
  };
};
