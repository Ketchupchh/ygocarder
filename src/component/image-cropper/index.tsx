...
export const isCropEqual = (
  cropL: Partial<ReactCrop.Crop>,
  cropR: Partial<ReactCrop.Crop>
) => {
  const {
    aspect: aspectL,
    height: heightL,
  ...
  if (
    typeof widthL === "number" &&
    typeof widthR === "number" &&
    Math.abs(widthL - widthR) > 0.01
  )
    return false;
  if (
    typeof heightL === "number" &&
    typeof heightR === "number" &&
    Math.abs(heightL - heightR) > 0.01
  )
    return false;
  if (
    typeof xL === "number" &&
  ...
  if (
    ...
    return false;
  if (
    typeof aspectL === "number" &&
    typeof aspectR === "number" &&
    Math.abs(aspectL - aspectR) > 0.01
  )
    return false;
...
const normalizeCrop = (
  ...
  if (unit === "px") {
    ...
    const isHeightRestricted =
      imageHeight / imageWidth >= oldHeightToWidthRatio;
  ...
  const acceptableError =
    (naturalHeight > naturalWidth ? naturalHeight : naturalWidth) * 0.05;
  const isRatioAcceptable =
    Math.abs(height * naturalHeight * ratio - width * naturalWidth) <=
    acceptableError;
  ...
  const normalizedX =
    oversizeRatio > 1 ? (100 - normalizedWidth) / 2 : x < 0 ? 0 : x;
...
export type ImageCropperRef = {
  ...
  forceSource: (
    type: "online" | "offline",
    artLinkOrData: string,
    cropInfo: Partial<ReactCrop.Crop>
  ) => void;
};
...
export const ImageCropper = forwardRef<ImageCropperRef, ImageCropper>(
  (
    ...
    const normalizedDefaultSource =
      defaultSourceType === "offline" ? "offline" : "online";
    ...
    const [sourceType, setSourceType] = useState<"offline" | "online">(
      normalizedDefaultSource
    );
    const [inputMode, setInputMode] = useState<"offline" | "online">(
      normalizedDefaultSource
    );
    ...
    const onLoad = useCallback(
      (img: HTMLImageElement) => {
        ...
        if (
          img.src === pendingCrop.current.source &&
          pendingCrop.current.crop
        ) {
          const internalId = pendingId.current;
          setTimeout(() => {
            if (internalId !== pendingId.current || !pendingCrop.current.crop)
              return;
    ...
    useEffect(() => {
      ...
      let expectedDrawWidth = Math.floor(
        (completedCrop.width ?? 0) *
          (cropUnit === "px" ? zoomX : naturalWidth / 100)
      );
      ...
      let drawCoordinateY = Math.min(
        ...
        Math.floor(
          (completedCrop.y ?? 0) *
            (cropUnit === "px" ? zoomY : naturalHeight / 100)
        )
      );
      ...
      if (
        ...
        const prominentSide =
          ratio * naturalHeight > naturalWidth ? "width" : "height";
        /**
    ...
    useImperativeHandle(forwardedRef, () => ({
      isLoading: () => isLoading,
      hasImage: () =>
        (typeof internalSource === "string" &&
          internalSource.length > 0 &&
          sourceType === "offline") ||
      ...
      forceSource: (type: "online" | "offline", source, cropInfo) => {
        const currentSource =
          sourceType === "offline" ? internalSource : externalSource;
        setInteracted(false);
    ...
    const hasImage =
      (sourceType === "offline" && (internalSource ?? "").length > 0) ||
    ...
    return (
      <div className={mergeClass("card-image-cropper", className)}>
        <div className="card-image-source-input">
          <div className="card-image-source-input-container">
            <div className="card-image-source-input-title">
              <span className="field-title">
                ...
                <IconButton
                  ...
                  containerProps={{
                    className: isDownloadable ? "" : "disabled",
                  }}
                  tooltipProps={{
            ...
            <div
              className={[
                "card-image-input",
                inputMode === "online" ? "" : "input-inactive",
              ].join(" ")}
            >
              <Input
                key="key"
            ...
            <div
              className={[
                "card-image-input",
                inputMode === "offline" ? "" : "input-inactive",
              ].join(" ")}
            >
              <Input
                ref={fileInputRef}
        ...
        >
          ...
          {hasImage && !error && (
            <div className="card-image-option">
              <Tooltip
                placement="left"
                overlay={
                  forceFit
              ...
              >
                <div
                  className={mergeClass(
                    "image-option force-fit-option",
                    forceFit ? "option-active" : ""
                  )}
                  onClick={() => {
                    setInteracted(true);
              ...
              {!forceFit && (
                <Tooltip
                  placement="left"
                  overlay={
                    language["image-cropper.button.center-vertical.tooltip"]
                  }
                >
                  <div
                    className="image-option alignment-center-option"
                    onClick={() => {
                      setInteracted(true);
                      setCrop((cur) => {
              ...
              {!forceFit && (
                <Tooltip
                  placement="left"
                  overlay={
                    language["image-cropper.button.center-horizontal.tooltip"]
                  }
                >
                  <div
                    className="image-option alignment-middle-option"
                    onClick={() => {
                      setInteracted(true);
                      setCrop((cur) => {
                        const { height, y } = cur.completed ?? {};

                        if (typeof height !== "number" || typeof y !== "number")
                          return cur;
                        const newCrop: ReactCrop.Crop = {
                          ...cur.completed,
                          y: (100 - height) / 2,
          ...
          {(!hasImage || (error && crossorigin === undefined)) &&
            !isLoading && (
              <Empty
          ...
          <ReactCrop
            key={`${sourceType}-${isMigrated}-${redrawSignal}`}
