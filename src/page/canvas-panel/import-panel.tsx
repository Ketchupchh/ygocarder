...
export const ImportPanel = forwardRef<ImportPanelRef, ImportPanel>(
  ({ allowHotkey, language, onImport, onClose }, ref) => {
    ...
    const startImport = async (
      ...
      try {
        if (cardData) {
          const { card: decodedCard, isPartial } = decodeCard(
            cardData,
            mode === "replace" || mode === "new"
              ? undefined
              : useCard.getState().card
          );
          ...
          if (
            imageSurvey &&
            decodedCard.artSource === "online" &&
            isYgoprodeckImage(decodedCard.art)
          ) {
      ...
      try {
        ...
        if (target) {
          ...
          /** Potential crushed data */
            uncrushedValue.startsWith("{") &&
            uncrushedValue.endsWith("}")
          ) {
            cardData = JSON.parse(uncrushedValue);
          } else {
            const ygoproDeckApi = normalizedValue.startsWith(
              "https://db.ygoprodeck.com/api"
            )
            ...
            const baseCardData: { data: YgoproDeckCard[] } =
              await response.json();
            ...
            cardData =
              ...
              baseCardData.data.sort((l, r) => {
                ...
                const viewWeekRatio =
                  (lMisc.viewsweek + 1) / (rMisc.viewsweek + 1);
    ...
    return (
      <>
            ...
            {mode !== "merge" ? (
              <div className="prompt-alert">
                ...
                <span className="alert">
                  {language["prompt.import.instruction.alert.line-1"]}
                </span>
              </div>
            ) : null}
            ...
            {mode !== "merge" ? (
              <div className="import-container-upload">
                <span className="option-separator">
                  {language["prompt.import.alternative.label"]}
                </span>
                <StyledActionIconButton
        ...
        <Tooltip overlay={language["button.import.tooltip"]}>
          ...
          >
            <input
              key={`direct-file-input-${inputKey}`}
