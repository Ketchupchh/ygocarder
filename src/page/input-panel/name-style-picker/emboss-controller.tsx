...
export const EmbossController = forwardRef<
  EmbossControllerRef,
  EmbossController
>(
  (
        ...
        setTimeout(() => {
          if (relevant) {
            ...
            if (
              ...
              Math.abs(nextAngle[0] - currentAngleVec.current[0]) >
                VECTOR_VALUE_EPSILON ||
              Math.abs(nextAngle[1] - currentAngleVec.current[1]) >
                VECTOR_VALUE_EPSILON ||
              Math.abs(nextAngle[2] - currentAngleVec.current[2]) >
                VECTOR_VALUE_EPSILON ||
              thickness !== currentThickness.current
            ) {
              onChange({
                lightColorVec: nextColor,
                lightAngleVec: nextAngle,
                yaw: angle,
                pitch: verticalAngle,
                thickness,
              });
    ...
    return (
      <EmbossControlContainer
        className={mergeClass(ANGLE_PICKER_CLASSNAME, "emboss-control")}
      >
        <div className="emboss-control-title">{children}</div>
        <div className="emboss-control-left">
          <h2>
            {language["input.name-style.emboss.color.label"]}&nbsp;&nbsp;
            <MaterialColor
          ...
          {language["input.name-style.emboss.color.alert"](
            <span
              key="color-navigate"
          ...
          <h2>
            {language["input.name-style.emboss.pitch.label"]}&nbsp;&nbsp;
            <SyncOutlined
          ...
          <h2>
            {language["input.name-style.emboss.yaw.label"]}&nbsp;&nbsp;
            <SyncOutlined
              className="reset-button"
              onClick={() => {
