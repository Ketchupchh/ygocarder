...
export const FooterInputGroup = forwardRef<
  FooterInputGroupRef,
  FooterInputGroup
>(({ isMonster, showCreativeOption, onTakePicker }, ref) => {
  ...
  const showDefInput = showLinkInput ? showDefAndLink : true;
  const showPadding =
    [showAtkInput, showDefInput, showLinkInput].filter(
      (entry) => entry === true
    ).length %
      2 ===
    1;
  const copyrightList =
    format && copyrightMap[format as keyof typeof copyrightMap]
      ? copyrightMap[format as keyof typeof copyrightMap]
      : copyrightMap.tcg;

  const changeATK = useMemo(
    () =>
      getUpdater("atk", (value) => (typeof value === "string" ? value : value)),
    [getUpdater]
  );
  const changeDEF = useMemo(
    () =>
      getUpdater("def", (value) => (typeof value === "string" ? value : value)),
    [getUpdater]
  );
  const changeLinkRating = useMemo(
    () =>
      getUpdater("linkRating", (value) =>
        typeof value === "string" ? value : value
      ),
    [getUpdater]
  );
  const changePassword = useMemo(
    () => getUpdater("password", undefined, "debounce"),
    [getUpdater]
  );
  const onStickerChange = useMemo(() => getUpdater("sticker"), [getUpdater]);
  const changeCreator = useMemo(
    () => getUpdater("creator", undefined, "debounce"),
    [getUpdater]
  );
  ...
  useImperativeHandle(ref, () => ({
    setValue: ({ password, creator, atk, def, linkRating }) => {
      ...
      if (typeof password === "string")
        passwordInputRef.current?.setValue(password);
      if (typeof creator === "string")
        creatorInputRef.current?.setValue(creator);
      if (typeof linkRating === "string")
        linkRatingInputRef.current?.setValue(linkRating);
    },
  }));

  return (
    <StyledFooterInputContainer className="card-footer-input">
      {(isMonster || showCreativeOption) && (
        <>
          ...
          {showLinkInput && (
            <CardTextInput
              ref={linkRatingInputRef}
              id="link"
              addonBefore={
                <StyledLinkRatingInputContainer className="input-label-with-button">
                  <div className="input-label">
                    {language["input.link.label"]}
                  </div>
                  <Tooltip title={language["input.link.auto.tooltip"]}>
                    <div className="auto-link-rating-input">
                      {typeof linkRating === "string" &&
                      linkRating.length > 0 ? (
                        <Checkbox
                          onChange={(e) => {
                            if (e.target.checked) {
      ...
      <CardTextInput
        ...
        id="password"
        addonBefore={
          <StyledInputLabelWithButton className="input-label-with-button">
            <div className="input-label">
              {language["input.password.label"]}
            </div>
            <IconButton
              onClick={() =>
                passwordInputRef.current?.setValue(randomPassword())
              }
              Icon={SyncOutlined}
      ...
      <CardTextInput
        ...
        addonBefore={
          <StyledInputLabelWithButton className="input-label-with-button">
            <div className="input-label">
              {language["input.copyright.label"]}
            </div>
            <Dropdown
            ...
            >
              <div>
                <IconButton onClick={() => {}} Icon={UnorderedListOutlined} />
              </div>
            </Dropdown>
          </StyledInputLabelWithButton>
        }
