...
type OtherFinishPicker = {
  ...
  changeFinish: (
    valueMap: Record<keyof typeof OtherFinishTypeMap, string>
  ) => void;
...
const OtherFinishPicker = ({
  ...
  const [selectedType, setSelectedType] = useState<
    keyof typeof OtherFinishTypeMap
  >(OtherFinishTypeMap["art"].key);
  ...
  const activeWidget = Object.values(valueMap).some(
    (entry) => entry !== "normal"
  );
  ...
  const IconDropdown = (
      ...
      content={
        <div className="overlay-event-absorber">
          <StyledDropdown.Container>
            <StyledDropdownLabel>
              {language["input.other-finish.label"]}
            </StyledDropdownLabel>
            {finishList.map(({ label, value }) => {
              return (
                ...
                >
                  <div className="value">
                    <div className="tag">
                      {valueMap[value] === "normal" || valueMap[value] === ""
                        ? "Auto"
                        : valueMap[value]}
                    </div>
                  </div>
    ...
    >
      <span
        className={`field-title card-icon-dropdown ${
          activeWidget ? "active" : ""
        } ${showCreativeOption ? "" : "disabled"}`}
      >
        {IconDropdownLabel}
      </span>
    </Popover>
...
export const ImageInputGroup = forwardRef<ImageInputGroupRef, ImageInputGroup>(
  (
    ...
    const changeLinkStatus = useCallback(
      (willBecomeLink: boolean) =>
        setCard((currentCard) => {
          return {
            ...currentCard,
    ...
    const changeArtFit = useCallback(
      (status: boolean) =>
        setCard((currentCard) => {
          return {
            ...currentCard,
    ...
    const changeArtSource = useMemo(
      () => getUpdater("artSource"),
      [getUpdater]
    );
    const changeArtFinish = useMemo(
      () => getUpdater("artFinish"),
      [getUpdater]
    );
    const changeOtherFinish = useMemo(
      () => getUpdater("otherFinish"),
      [getUpdater]
    );
    const changeImageCrop = useCallback(
      (
        cropInfo: Partial<ReactCrop.Crop>,
        sourceType: "offline" | "online",
        byUser?: boolean
    ...
    return (
      ...
      >
        ...
        >
          <LinkMarkerPicker
            key={`link-${JSON.stringify(linkMap)}`}
            active={isLink === true}
