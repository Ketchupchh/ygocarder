...
export const StarTypeSelector = ({
  ...
  const filterFunction = showMixableOnly
    ? (entry: IconTypeInfo) => entry.isMixable
    : () => true;

  return (
    <IconDropdownContainer
      className={mergeClass(
        "icon-dropdown-container overlay-event-absorber",
        disabled ? "selector-disabled" : "",
        className
      )}
    >
      {disabled && (
        <IconDropdownOverlay className="container-disable-overlay" />
      )}
      <StyledIconDropdown className="container-group container-star">
        {showRemove && (
          <StyledDropdown.Option onClick={() => onRemove?.()}>
            <CloseCircleOutlined />
            &nbsp;{language["input.icon-type.remove.label"]}
          </StyledDropdown.Option>
        )}
        {IconTypeList.filter(filterFunction).map(
          ({ fullLabelKey, value, icon }) => {
            return (
              <StyledDropdown.Option
                key={value}
      ...
      <StyledIconDropdown className="container-group container-attribute">
        {IconTypeAttributeList.filter(filterFunction).map(
          ({ fullLabelKey, value, icon }) => {
            return (
              <StyledDropdown.Option
                key={value}
      ...
      <StyledIconDropdown className="container-group container-icon">
        {IconTypeStList.filter(filterFunction).map(
          ({ fullLabelKey, value, icon }) => {
            return (
              <StyledDropdown.Option
                key={value}
...
export type StarPicker = {} & WithLanguage;
export const StarPicker = forwardRef<StarPickerRef, StarPicker>(
  ({ language }, ref) => {
    const { starList, setCard } = useCard(
      useShallow(({ card: { starList }, setCard }) => ({
        starList,
        setCard,
      }))
    );
    const [visible, setVisible] = useState(false);
    const changeStarList = useMemo(
      () => (transformer: string[] | ((prev: string[]) => string[])) => {
        setCard((card) => {
          const nextStarList = Array.isArray(transformer)
            ? transformer
            : transformer(card.starList);
    ...
    const [{ slotList, use12 }, setSlot] = useState(() => {
    ...
    const { activeList } = activeInformation;
    return (
      <Modal
              ...
              if (e.target.checked) {
                setSlot(({ slotList }) => {
                  const nextSlotList: typeof slotList = [
                    ...
                    {
                      disabled: true,
                      index: MAX_STAR_LENGTH - 1,
                      slot: `slot-${MAX_STAR_LENGTH - 1}`,
                    },
                  ];
                ...
                changeStarList((currentStarList) =>
                  currentStarList.slice(0, NORMAL_STAR_LENGTH)
                );
                setActiveInformation(({ activeList }) => ({
                  activeList: activeList.filter(
                    (entry) => entry !== MAX_STAR_LENGTH - 1
                  ),
                }));
              } else {
                setSlot(({ slotList }) => {
                ...
                changeStarList((currentStarList) =>
                  padRight(currentStarList, MAX_STAR_LENGTH)
                );
              }
            }}
          >
            {language["input.icon-type.slot.use-12.label"]}
          </Checkbox>
          <Button
            size="small"
            onClick={() => {
              setActiveInformation((cur) => {
                const nextActiveList = slotList
                  .filter(
                    (entry) =>
                      !cur.activeList.includes(entry.index) && !entry.disabled
                  )
                  ...
                  <Checkbox
                    ...
                    onChange={(e) => {
                      ...
                      if (willChecked) {
                        ...
                        setActiveInformation((cur) => {
                          const nextActiveList = cur.activeList.filter(
                            (entry) => entry !== index
                          );
        ...
        <StarListControlPanel>
          <Button
            ...
            onClick={() => {
              changeStarList((currentStarList) => {
                return [
                  ...currentStarList.slice(1, currentStarList.length),
                  "",
                ];
              });
              setActiveInformation({ activeList: [] });
          ...
          >
            <DoubleLeftOutlined />
            &nbsp;{language["input.icon-type.slot.shift-left.label"]}
          </Button>
          <Button
            size="small"
            onClick={() => {
              changeStarList((currentStarList) => {
                return [
                  "",
                  ...currentStarList.slice(0, currentStarList.length - 1),
                ];
              });
              setActiveInformation({ activeList: [] });
            }}
          >
            {language["input.icon-type.slot.shift-right.label"]}&nbsp;
            <DoubleRightOutlined />
          </Button>
        </StarListControlPanel>
        <StarTypeSelector
